...
-- Função para pegar a hora atual em formato 24 horas (sem segundos)
local function getCurrentTimeFormatted()
    local time = os.date("*t")  -- Obtém a data e hora atual
    local hour = time.hour
    local minute = time.min

    -- Ajusta minutos para dois dígitos
    minute = string.format("%02d", minute)

    return string.format("%02d:%02d", hour, minute)  -- Exibe no formato 24h
end
...
-- Adicionar Labels para exibir as informações de tempo
local TimeLabel = Tab:AddLabel("Time: " .. getCurrentTimeFormatted())
local TimeInGameLabel = Tab:AddLabel("Time in Game: 00 days 00:00:00")

-- Função para atualizar a hora e o tempo no jogo
local function UpdateTime()
    TimeLabel:Set("Time: " .. getCurrentTimeFormatted())  -- Atualiza a hora atual (sem segundos)
    TimeInGameLabel:Set("Time in Game: " .. getTimeInGame())  -- Atualiza o tempo no jogo
end

-- Atualiza as estatísticas e os tempos a cada segundo
